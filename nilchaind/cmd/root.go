...
func NewRootCmd() *cobra.Command {
	...
	tempApp := nillionapp.NewNillionApp(log.NewNopLogger(), dbm.NewMemDB(), nil, true, simtestutil.NewAppOptionsWithFlagHome(tempDir()))
...
func initAppConfig() (string, interface{}) {
	...
	// WASMConfig defines the configuration for the wasm module.
	...
	//
	// In nillionapp, we set the min gas prices to 0.
...
func initRootCmd(rootCmd *cobra.Command, encodingConfig params.EncodingConfig, basicManager module.BasicManager) {
	...
	rootCmd.AddCommand(
		genutilcli.InitCmd(basicManager, nillionapp.DefaultNodeHome),
		debug.Cmd(),
		confixcmd.ConfigCommand(),
		pruning.Cmd(newApp, nillionapp.DefaultNodeHome),
		snapshot.Cmd(newApp),
		server.QueryBlockResultsCmd(),
	)

	server.AddCommands(rootCmd, nillionapp.DefaultNodeHome, newApp, appExport, addModuleInitFlags)
...
func newApp(
	...
	return nillionapp.NewNillionApp(
...
// appExport creates a new nillionapp (optionally at a given height) and exports state.
func appExport(
	logger log.Logger,
	db dbm.DB,
	traceStore io.Writer,
	...
	var nillionApp *nillionapp.NillionApp
	...
	if height != -1 {
		nillionApp = nillionapp.NewNillionApp(logger, db, traceStore, false, appOpts)

		if err := nillionApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		nillionApp = nillionapp.NewNillionApp(logger, db, traceStore, true, appOpts)
	}

	return nillionApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs, modulesToExport)
}

var tempDir = func() string {
	dir, err := os.MkdirTemp("", "nillionapp")
	if err != nil {
		dir = nillionapp.DefaultNodeHome
	}
